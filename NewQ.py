...
def NewQ():
    ...
    global x
    x = 0
    create_task_button = Button(window, text="Create Task", command=lambda: [give_to_database(name_entry.get(), description_entry.get("1.0", END), creation_date_entry.get(), due_date_entry.get(), employee_entry.get(), window,x)])
...
def give_to_database(name_entry,description_entry,creation_date_entry,due_date_entry, employee_entry,window,x):
    print(x)
    if x == 0:
        p = Database_tasks.insert_task(name_entry, description_entry, creation_date_entry, due_date_entry, employee_entry)
        window.lift()
        if p != 0 and p != 1:
            window.destroy()
    else:
        Database_tasks.add_task_for_team(employee_entry,name_entry,description_entry,due_date_entry)
